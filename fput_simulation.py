...
!!! Using wikipedia eqn instead
"""
import dearpygui.dearpygui as dpg
import numpy as np

dpg.create_context()
dpg.create_viewport(title='FPUT Simulation (Wikipedia formula)', width=1150, height=700)
...
def update_linear_model():
    ...
    for j in range(1,n):
        ddot_q[j] = -spring_constant*(2*q_linear[j]-q_linear[j+1]-q_linear[j-1]) 
...
def update_nonlinear_model():
    global q_nonlinear, dot_q_nonlinear, dt, n, alpha, spring_constant
    ddot_q = np.zeros_like(q_nonlinear)
    for j in range(1,n):
        ddot_q[j] = -spring_constant*(2*q_nonlinear[j]-q_nonlinear[j+1]-q_nonlinear[j-1])*(1+alpha*(q_nonlinear[j+1]-q_nonlinear[j-1]))
...
def calculate_fput_H(q,q_dot):
    global alpha, spring_constant
    kinetic = np.sum(q_dot**2)/2
    potential = np.sum(spring_constant/2*(q[1:]-q[:-1])**2+spring_constant*alpha/3*(q[1:]-q[:-1])**3)
...
def save_values():
    global n, alpha, spring_constant, dt, x, q_linear, dot_q_linear, q_nonlinear, dot_q_nonlinear, FRAME, frame_update_rate
    global x_time, initial_energy, linear_energy, nonlinear_energy
    n = int(dpg.get_value("slider_n"))
    alpha = dpg.get_value("slider_alpha")
    spring_constant = dpg.get_value("slider_spring")
...
with dpg.window(label="Parameters", pos=(0,0), width=450,height=650):
    ...
    dpg.add_slider_float(label="Alpha", default_value = alpha, min_value=-1.5, max_value=1.5, tag = 'slider_alpha')
    dpg.add_slider_float(label='Spring Constant', default_value = spring_constant, min_value=0, max_value=10, tag = 'slider_spring')
...
def checkbox_toggle(sender, app_data, user_data):
    ...
    dpg.configure_item("linear fput energy", show=app_data)
